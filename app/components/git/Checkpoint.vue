...
async function save() {
  ...
  saving.value = true
  const ok = await git.checkpointSave(git.selectedRepo, name.value.trim() || undefined as any)
  saving.value = false
  if (ok) {
    name.value = ''
...
<template>
  <div class="panel-card overflow-hidden">
    ...
    >
      ...
      Checkpoints
      <span v-if="git.checkpoints.length" class="font-mono text-dimmed">
        {{ git.checkpoints.length }}
      </span>
      <UTooltip text="Save a named snapshot before letting AI make big changes" :content="{ side: 'top' }">
        <UIcon name="i-lucide-info" class="size-3 text-dimmed ml-auto" />
      </UTooltip>
    </button>

    <div v-if="open" class="border-t border-(--ui-border-muted) p-2 space-y-2">
      <!-- Save new checkpoint -->
      ...
      <!-- Checkpoint list -->
      <div v-if="git.checkpoints.length" class="space-y-1">
        <div
        ...
        >
          <UIcon name="i-lucide-flag" class="size-3 text-purple-400 shrink-0" />
          <div class="flex-1 min-w-0">
            <p class="text-[11.5px] text-toned truncate">
              {{ cp.message.replace('checkpoint: ', '') }}
            </p>
            <p class="text-[10px] font-mono text-dimmed">{{ cp.branch }} · {{ cp.date }}</p>
          </div>
      ...
      <p v-else class="text-[11px] text-dimmed italic px-1">No checkpoints yet.</p>
    </div>
  </div>
</template>
